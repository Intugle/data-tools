...
class GraphSearch:
    ...
    async def get_shortlisted_tables(self, query: str):
        """
        ...
        _, traversal_path, search_result = await self.traverse_graph(
            query,
            self.graph,
            top_k=int(settings.NETWORKX_GRAPH_TOP_K_TABLE),
            max_depth=int(settings.NETWORKX_GRAPH_MAX_DEPTH_TABLE),
        )

        if not traversal_path:
            return pd.DataFrame()

        shortlisted_tables = search_result.loc[search_result.index.isin(traversal_path)]

        return shortlisted_tables
