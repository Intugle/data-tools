...
CONCEPTUAL_SEARCH_COLLECTION_NAME = "conceptual_search_columns"
...
def prepare_chunk_document(manifest):
    column_description_path = os.path.join(settings.COL_DESCRIPTION_DIR, "column_description.csv")
    ...
    if not column_description_path.exists():
        ...
        # existing = pd.read_csv(column_description_path)
        ...
        # remaining_columns = set(column_description_df["id"].unique()) - set(existing["id"].unique())
        
        # column_description_df = column_description_df[(column_description_df["business_glossary"] != "") & (column_description_df["id"].isin(list(remaining_columns)))].reset_index(drop=True)
        ...
        for i, data in tqdm(enumerate(batched(column_description_df, 30), 1), position=0, leave=True):
            ...
            # column_description_df.loc[data.index,"concepts"] = data["business_tags"].apply(clean_concepts)
            ...
            log.info(f"[*] Batch {i} completed ... sleeping for 60 seconds")
    ...
    # ["unique identifier","transaction","transaction code"]
