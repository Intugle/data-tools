...
class MCPTools:
    ...
    async def get_tables(self, session: ClientSession = None):
        """
        Testing getting tables from the mcp server
        Args:
            session (ClientSession, optional): mcp client session

        Returns:
            bool: flag to indicate whether test passed or not
        """
        tables = await session.call_tool(
            name=Tools.GET_TABLES,
        )

        assert isinstance(tables.structuredContent["result"], list)

        # assert len(tables.strip()) > 0, "No tables information fetched"

    @connection_decorator()
    async def get_schema(self, session: ClientSession = None):
        schemas = await session.call_tool(name=Tools.GET_SCHEMA, arguments={"table_names": ["orders", "products"]})
